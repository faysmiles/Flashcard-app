...
import re
import json
import requests# app.py - DROPDOWN TEXT NOW PURE BLACK (Always Readable)# app.py - CLEAN WORKING VERSION
# Copy this entire code into app.py (replace everything)
...
# ========== TOP BANNER ==========
emojis = colors.get("emojis", ["✨", "📚", "💡", "🎓", "📖", "⭐"])
emoji_row = "".join([f"<span>{e}</span>" for e in emojis])
st.markdown(f'<div class="top-banner">{emoji_row}</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    for scheme in COLOR_SCHEMES.keys():
        if st.button(scheme, use_container_width=True, key=f"theme_{scheme}"):
    ...
    st.markdown("### ✍️ Font")
    font = st.selectbox("Font Style", FONT_OPTIONS, index=FONT_OPTIONS.index(st.session_state.font_style), key="font_select")
    ...
    reading_level = st.selectbox("Level", list(READING_LEVELS.keys()), key="level_select")
...
input_type = st.radio("Input Type", ["Paste Text", "Upload File"], horizontal=True, key="input_type")

user_text = ""
if input_type == "Paste Text":
    user_text = st.text_area("Paste your text here...", height=150, placeholder="Paste any text...", label_visibility="collapsed", key="text_input")
else:
    uploaded = st.file_uploader("Upload TXT, PDF, or DOCX", type=["txt", "pdf", "docx"], label_visibility="collapsed", key="file_upload")
    if uploaded:
        try:
...
if st.session_state.generated and st.session_state.flashcards:
    ...
    if not is_flipped:
        st.markdown(f"""
        <div style='background: {colors['card_bg']}; border-radius: 20px; padding: 60px 40px; text-align: center; border-left: 6px solid {colors['accent']}; box-shadow: 0 8px 24px rgba(0,0,0,0.08); margin: 20px 0;'>
            <div style='font-size: 80px; margin-bottom: 20px;'>{card['emoji']}</div>
            <h2 style='margin: 20px 0; color: {colors['text']};'>{card['title']}</h2>
            <p style='margin-top: 30px; color: {colors['accent']}; font-size: 14px;'>Click "Reveal Facts" below</p>
        </div>
        """, unsafe_allow_html=True)
    else:
        facts_html = "".join([f"""
        <div style='display: flex; align-items: center; gap: 15px; padding: 15px; margin: 10px 0; background: {colors['bg']}; border-radius: 12px;'>
        ...
        st.markdown(f"""
        <div style='background: {colors['card_bg']}; border-radius: 20px; padding: 40px; border-left: 6px solid {colors['accent']}; box-shadow: 0 8px 24px rgba(0,0,0,0.08); margin: 20px 0;'>
    ...
    st.download_button("Download All Cards (Text)", text_export, "flashcards.txt", use_container_width=True, key="download_btn")
...
import streamlit as st# app.py - PURPLE THEME: PALE LAVENDER DROPDOWN WITH DARK TEXTimport streamlit as st
import os
import json
from dotenv import load_dotenv
from openai import OpenAI
